...
- Active navigation item indicators: Desktop links (`.nav__link.is-active`), services dropdown items (`.nav__menu-item.is-active`), and mobile navigation drawer items (`.nav__drawer-link.is-active`, `.nav__drawer-subitem.is-active`, `.nav__drawer-accordion-btn.is-current-page`) dynamically highlight the current active route with accent-tinted pill backgrounds, elevated soft border contrast, and active indicator styling in both light and dark modes.
